...
import { Table, Menu, ActionIcon, Pagination, Skeleton } from "@mantine/core";
...
const MyTaskList = () => {
  ...
  const { tasks, error, isLoading } = useSelector((state) => state.tasks);
  ...
  return (
    <div>
      {isLoading ? (
        <div className="md:h-[74vh] flex flex-col gap-5">
          {/* Skeleton Loading UI */}
          <Skeleton
            height={40}
            width="100%"
        ...
        <>
          {!tasks || tasks.length === 0 ? (
            <div className="md:h-[74vh] flex flex-col items-center justify-center gap-5">
              <h3 className="text-2xl md:text-3xl text-center text-gray-500">
